...
Translathons are 2-hour events designed as a combination of *translation* (the communication of the meaning of a source-language text by means of an equivalent target-language text) with the ways and energy of a *hackathon* (a dedicated period of time where participants make contributions to selected FOSS projects).
...
> We focus the translation effort to one resource: rOpenSci Packages: Development, Maintenance, and Peer Review.
> NumFocus provide the funding to pay mentors and editors a \$200 USD honorarium for participating in the translathons.
...
The timeline is setup for a single event, but repeat events are easy to insert after the first three tasks.
...
When you organize the translathon as part of another event, the date depended on the availability of both the conference schedule and the mentors and editors.
...
> In our pilot case, the translathon was held as a satellite event of the LatinR conference. The conference offer us two potential days and we offer those days with time slots to mentors and editor to set the final date. 
...
> This system was also used to send pre-event notifications and to share access information for the online meeting wich helps with the logistic of the event.
...
We made sure the differents projects are up todate for the mini translathons. Usually the PR listed under `Todo - First Review`
and `Todo - Second Review` are the one to work during the translathons.

rOpenSci staff create pull requests with the first automatic translation for the texts needing translation and label them with the translation corresponding tag and event-specific tag.

> `translation :mate:` is the label used for Spanish translations `Tradução` is the label used for Portuguese translations.

Translation issues should be:
...
-   Before
    -   Create [Coworking Document](https://docs.google.com/document/d/1juXuCMvz7a9BX_VK6SO4QCs7T78Bhttps://docs.google.com/document/d/1CjvRCIuX3gqJZ-_0_Nb29--LpP7PEMQdCiWTHVdH9XE/edit?usp=sharing) in the language of the event (prepopulate registrants)
    ...
    -   Sent reminder one week before and one hour before (email and social media)
-   During
    ...
    -   Sent the Slack invitation to the attendees and add them to the specific Slack channel.
-   After
